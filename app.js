...
// Safe wrapper for Lucide icon generation to prevent page crash if CDN is blocked
function safeCreateIcons() {
  if (typeof lucide !== 'undefined' && lucide.createIcons) {
    ...
    console.warn("Lucide library is not loaded/blocked.");
  }
}
...
function renderAboutAndStats() {
  ...
  if (appState.education && appState.education[0]) {
    ...
    if (appState.education[0].degree) {
      ...
      if (deg.toLowerCase().includes("m.c.a") || deg.toLowerCase().includes("mca")) {
        eduLabel = "M.C.A";
      } else if (deg.toLowerCase().includes("master")) {
...
// 6b. Publications Section
function renderPublicationsSection() {
  const container = document.getElementById("publications-container");
  if (!container) return;
  if (!appState.publications || appState.publications.length === 0) {
    container.innerHTML = `<p class="text-center text-muted">No publications added yet.</p>`;
    return;
  }

  let html = "";
  appState.publications.forEach(pub => {
    const linkHtml = pub.link && pub.link !== "#" 
      ? `<a href="${pub.link}" target="_blank" class="pub-link">Read Paper <i data-lucide="arrow-up-right"></i></a>`
      : "";
    ...
    html += `
      <div class="glass-card pub-card">
        <div class="pub-top">
          <h3>${pub.title}</h3>
          ${pub.publisher ? `<span class="pub-publisher"><i data-lucide="book-open"></i> ${pub.publisher}</span>` : ""}
        </div>
        <div class="pub-bottom">
          <span class="pub-date"><i data-lucide="calendar"></i> ${pub.date}</span>
          ${linkHtml}
        </div>
      </div>`;
  });
...
function renderContactSection() {
  ...
  if (info.whatsapp) {
    document.getElementById("contact-whatsapp").textContent = info.whatsapp;
    document.getElementById("contact-whatsapp-link").setAttribute("href", `https://wa.me/91${info.whatsapp}`);
  }
  
  if (info.instagram) {
    document.getElementById("contact-instagram").textContent = `@${info.instagram}`;
    document.getElementById("contact-instagram-link").setAttribute("href", `https://instagram.com/${info.instagram}`);
  }
...
function populateAdminForms() {
  ...
  // 7. Publications lists
  buildPublicationsForm();
}
...
// 6. Publications Nested Form
function buildPublicationsForm() {
  const container = document.getElementById("publications-edit-list");
  if (!container) return;
  container.innerHTML = "";
  
  if (!appState.publications) appState.publications = [];

  appState.publications.forEach((pub, index) => {
    const card = document.createElement("div");
    card.className = "edit-item-card";
    card.innerHTML = `
      <div class="edit-item-card-header">
        <span class="edit-item-title">Entry #${index + 1}: ${pub.title || "New publication"}</span>
        <span class="btn-delete-item delete-pub-btn" data-index="${index}">
          <i data-lucide="trash-2"></i> Delete
        </span>
      </div>
      <div class="form-group">
        <label>Publication Title</label>
        <input type="text" class="pub-input" data-index="${index}" data-key="title" value="${pub.title || ""}">
      </div>
      <div class="form-row">
        <div class="form-group">
          <label>Publisher / Journal</label>
          <input type="text" class="pub-input" data-index="${index}" data-key="publisher" value="${pub.publisher || ""}">
        </div>
        <div class="form-group">
          <label>Date (e.g. Sep 12, 2025)</label>
          <input type="text" class="pub-input" data-index="${index}" data-key="date" value="${pub.date || ""}">
        </div>
      </div>
      <div class="form-group">
        <label>Link URL (or '#' if none)</label>
        <input type="text" class="pub-input" data-index="${index}" data-key="link" value="${pub.link || "#"}">
      </div>`;
  ...
  // Add Item Button Listener
  const addBtn = document.getElementById("btn-add-pub");
  if (addBtn) {
    const cleanAddBtn = addBtn.cloneNode(true);
    addBtn.parentNode.replaceChild(cleanAddBtn, addBtn);
    
    cleanAddBtn.addEventListener("click", () => {
      appState.publications.push({
  ...
  document.querySelectorAll(".delete-pub-btn").forEach(btn => {
    btn.addEventListener("click", (e) => {
      const idx = parseInt(btn.getAttribute("data-index"));
      appState.publications.splice(idx, 1);
      saveStateLocally();
      renderPublicationsSection();
      buildPublicationsForm();
    });
  ...
  // Bind input changes
  document.querySelectorAll(".pub-input").forEach(input => {
    input.addEventListener("input", (e) => {
      const idx = parseInt(e.target.getAttribute("data-index"));
      const key = e.target.getAttribute("data-key");
      appState.publications[idx][key] = e.target.value;
      saveStateLocally();
      renderPublicationsSection();
    });
  });
